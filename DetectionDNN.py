...
covi = []
lp = [[0 for i in range(0,128)] for j in range(0,128)]
with os.scandir('C:\\Users\\cspde\\facial_dataset') as entries:
    ff = []
    for entry in entries:
        faces_vector=[]
        with os.scandir('C:\\Users\\cspde\\facial_dataset\\'+entry.name) as imgs:
            for img1 in imgs:
                tm = cv.TickMeter()
                img1 = cv.imread('C:\\Users\\cspde\\facial_dataset\\'+entry.name+"\\"+img1.name)
                img1Width = 100
                img1Height = 100
        ...
        for i in faces_vector:
            ff.append(i)
    #with open('names7.csv', 'w',newline = '') as csvfile:
        #csvwriter = csv.writer(csvfile)
        #csvwriter.writerows(ff)
        if len(faces_vector) > 1:
            covi.append(np.cov(faces_vector,rowvar = False))
for row in range(0,128):
    for col in range(0,128):
            for cov in range(0,len(covi)):
                lp[row][col] += covi[cov][row][col]
                lp[row][col] /= 9
print(lp)
with open('cov1.csv', 'w',newline = '') as csvfile:
        csvwriter = csv.writer(csvfile)
        csvwriter.writerows(lp)                    
cv.waitKey(0)
